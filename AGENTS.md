...
API.** There is no frontend framework. Content lives in a SQLite database at `data/srn.db`, which
is gitignored runtime state: it is never committed, and a fresh deployment starts empty with one
admin account.
...
One file: `data/srn.db` (better-sqlite3, WAL mode). Collections keep their array shape as JSON
documents in a single `collections` table, behind `server/store.js`'s `read`/`write`/`update`.
`npm run seed` bootstraps an empty datastore plus one admin; the server also creates the admin on
first boot if the datastore has no users. `js/data.js` remains the frontend's static-host fallback
and is never written to.
- **One runtime dependency: `better-sqlite3`** (node:sqlite needs Node 22; this project runs on
  Node 20). `vite` and `jsdom` are devDependencies: a build tool and the test DOM.
...
├── data/                  <- THE DATABASE. srn.db (SQLite), gitignored runtime state.
...
- **`data/` must never be committed.** `srn.db` (and its WAL files) hold live sessions and
  everything users posted; the directory is gitignored in full.
- **The first admin password comes from `SRN_ADMIN_PASSWORD` or is generated and printed once.** It
  is not stored in plaintext. To change it: `npm run reset-admin -- admin@srn.ng`, or delete
  `data/srn.db` and re-seed with `SRN_ADMIN_PASSWORD=...`.
